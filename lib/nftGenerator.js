...
/**
 * Generate NFT metadata from traits (standards-compliant ERC721 metadata)
 * Uses raw trait values for attributes
 * @param {object} traits - The trait values
 * @param {number} seed - The deterministic seed
 * @returns {object} Metadata object
 */
export function generateMetadata(traits, seed) {
  // Build attributes array with trait_type and value
  const attributes = [
    { trait_type: 'Core Type', value: traits.coreType },
    { trait_type: 'Edge Type', value: traits.edgeType },
    { trait_type: 'Energy Mode', value: traits.energyMode },
    { trait_type: 'Accent Geometry', value: traits.accentGeometry },
    { trait_type: 'Light Burst', value: traits.lightBurst },
    { trait_type: 'Rarity Tier', value: traits.rarityTier },
    { trait_type: 'Seed', value: String(seed) },
  ]

  return {
    name: `NovaTok Core #${seed}`,
 ...
 * @returns {Promise<object>} NFT data including imageDataUrl, metadata, traits, seed
 */
export async function generateNFT(traits, size = 1024) {
  ...
  const metadata = generateMetadata(traits, seed)
